function [comError,fNoQp,f_HDot,NA,tauModel,Sigmaf_HDot,SigmaNA,...
    ...
    %% CoM and seesaw angle references
    amplitude              = 0*pi/180;
    frequency              = 0.5;
    ...
    thetaDot_seesaw_des    = amplitude*frequency*2*pi*cos(2*pi*frequency*t);
   
    theta_seesaw_rad       = atan2(w_R_s(3,2),w_R_s(2,2));
    yCoM_des               = controlParams.references.xcomDes(2); % (mT*(gain.seesawKLambda*theta_seesaw_rad*seesaw.rho)-mS*w_p_com_s(2))/mR;
    posCoM_desired         = [controlParams.references.xcomDes(1); yCoM_des; controlParams.references.xcomDes(3)]; 
   
    psat                   = saturate(controlParams.gain.PCOM * (posCoM_desired - w_p_com_R(1:3)),...
                                     -controlParams.gain.P_SATURATION, controlParams.gain.P_SATURATION);
...
%         pinvAs          = pinvDamped(As,reg.pinvDampA);
        ...
        desiredDyn_s    = -gain.seesawKP*(theta-theta_seesaw_des*180/pi)-gain.seesawKD*(thetaDot-thetaDot_seesaw_des*180/pi);
        
        f_HDot_r        = pinvAr* (desiredDyn_r - gravityWrench);
        
        if CONFIG.USE_PASSIVE_CONTROL == 1 
            
            f_HDot_s        = -gain.seesawKP_passive*transpose(Aseesaw*NAr)*(theta-theta_seesaw_des*180/pi)...
                              -gain.seesawKD_passive*transpose(Aseesaw*NAr)*(thetaDot-thetaDot_seesaw_des*180/pi);
        else
...
%         pinvAs          = pinvDamped(As,reg.pinvDampA);
        ...
        if CONFIG.USE_PASSIVE_CONTROL
            f_HDot_s        = -gain.seesawKP_passive*transpose(Aseesaw)*(theta-theta_seesaw_des*180/pi)...
                              -gain.seesawKD_passive*transpose(Aseesaw)*(thetaDot-thetaDot_seesaw_des*180/pi);
        else
            f_HDot_s        = pinvAseesaw*desiredDyn_s;
        end
        
        f_HDot_r        = pinv(Ar*NAs,0.0001)*(desiredDyn_r - gravityWrench -Ar*f_HDot_s);
        f_HDot          = f_HDot_s + NAs*f_HDot_r;
