...
dr_s           = seesaw.rho * S(omega_s) * s_R_w * e3; %derivative of r_s
...
CentroidalMat  = [eye(3), zeros(3), eye(3), zeros(3);
                  S(robotLeftFoot-robotCoM), eye(3), S(robotRightFoot-robotCoM), eye(3)];
...
if seesaw.kind == 1 %Spherical seesaw
    ...
    Omega_1    = (1/det(Theta))*seesaw.iota*Theta*S(r_s)*(S(dr_s) * omega_s - S(omega_s)^2 * r_s - g_s) ...
                     -Omega_0*S(omega_s)*seesaw.invIota*omega_s;
...
tauModel       = LambdaPinv * (J / Mass * genBiasForces ...
               ...
               + DeltaDot * omega_s + blkdiag(S(omega_s),S(omega_s),S(omega_s),S(omega_s)) * Delta * omega_s) ...
