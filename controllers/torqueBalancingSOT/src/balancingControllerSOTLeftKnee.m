...
function [hessianMatrix,biasVector,constraintMatrixLeftFoot,constraintMatrixRightFoot,constraintMatrixEq, upperBoundEqConstraints] = ...
                                        balancingControllerSOTLeftKnee(constraints, impedances, dampings, jointAngles, ...
    ...
    %Temporary fix to avoid the left leg joint limit
    if state == 2 || state == 3
        i = 15; %left knee is at index 15
        qmax = 5*pi/180;
        qmin = -100*pi/180;
        delta = (qmin - qmax)/2;
        q0 = (qmin + qmax)/2;
        xi = atanh((jointAngles(i) - q0)/delta);
        desXi = atanh((desJointAngles(i) - q0)/delta);
        dxi = robotVelocity(7+i)/(delta * sech(xi)^2);
        tauFeedback(i) = impedances(i) * (xi - desXi) + dampings(i) * dxi;
    end
    %
